...
- [x] Governing documents reconciled (step 7): F1 fixed inline, commit `4df5f25` on the
  feature branch. Board regenerated by the move.
- [ ] Remaining-tickets impact sweep (step 8): not run — the ticket does not conclude this
  pass (blocking finding routes to rework first); will run at the concluding re-review.
- [x] Summary presented below. No commit/MR to approve yet — publishing waits for the
  concluding review.
