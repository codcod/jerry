...
[group('build')]
build:
    go build -ldflags "-X main.Version={{version}}" -o {{bin}} ./cmd/jerry
...
# Regenerate every golden fixture, then re-run the suite
[group('qa')]
test-update:
    go test ./... -update
    go test ./...

# Static checks: formatting + go vet (matches CI)
[group('qa')]
lint:
    @unformatted="$(gofmt -l .)"; \
