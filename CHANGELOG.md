...
- `jerry comment` runs `related` over the merge request's changed files (`--base`, default
  ...
  exit 0, when nothing matches. Meant to run in CI: an absent or insufficiently-scoped
  `GITHUB_TOKEN` also degrades to a silent no-op rather than failing the pipeline, since a docs
  tool must never be the reason a merge request cannot merge. Every successful post appends one
  line to `jerry-adoption.jsonl` at the repo root — a new file that will appear in a repository's
